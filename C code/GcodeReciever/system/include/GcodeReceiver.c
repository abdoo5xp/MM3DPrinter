...
static void App_CallBack(Event_enu_t Event_Id){


	/*This is the only thing that changes the state */
	if (Event_Id == RX_Event){
		ReceivedCbf();
	}
...
//			Uart_RecvBuff(UART_INSTANCE, GcodeData, GcodeBytesCount);
			Uart_Recive(USART_2, GcodeData, GcodeBytesCount);
...
//	CountOfDataBytes[COUNT_OF_DATA_BYTES_NUMBER] = '\0';
	...
	RCC_ControlAHB1PeriClk(GCODERECEIVER_GPIO_CLK, RCC_PREPH_ENABLE);

	/*config rx pin , tx pin and uart channel*/
	GPIO_InitPin(&GcodeReceiver_Uart_Module[GCODERECEIVER_SELECTED_MODULE_IDX].tx_pin_cfg);
	GPIO_InitPin(&GcodeReceiver_Uart_Module[GCODERECEIVER_SELECTED_MODULE_IDX].rx_pin_cfg);

	/*Uart channel clock enable and NVIC selection */
	switch(UART_INSTANCE)
	{
	case USART_1:
		RCC_ControlAPB2PeriClk(RCC_APB2_PREPH_USART1, RCC_PREPH_ENABLE);
		Uart_IRQchannel = _USART1_IRQn;
		break;
	case USART_2:
		RCC_ControlAPB1PeriClk(RCC_APB1_PREPH_USART2, RCC_PREPH_ENABLE);
		Uart_IRQchannel = _USART2_IRQn;
		break;
	case USART_3:
		RCC_ControlAPB1PeriClk(RCC_APB1_PREPH_USART3, RCC_PREPH_ENABLE);
		Uart_IRQchannel = _USART3_IRQn;
		break;
	case USART_6:
		RCC_ControlAPB2PeriClk(RCC_APB2_PREPH_USART6, RCC_PREPH_ENABLE);
		Uart_IRQchannel = _USART6_IRQn;
		break;
	case UART_4:
		RCC_ControlAPB1PeriClk(RCC_APB1_PREPH_USART4, RCC_PREPH_ENABLE);
		Uart_IRQchannel = _UART4_IRQn;
		break;
	case UART_5:
		RCC_ControlAPB1PeriClk(RCC_APB1_PREPH_USART5, RCC_PREPH_ENABLE);
...
//	Uart_SetRecvCbf(GcodeReceiver_Uart_Module[GCODERECEIVER_SELECTED_MODULE_IDX].Uart_Channel, ReceivedCbf);
	Uart_Set_CallBack(App_CallBack);
...
//	Uart_RecvBuff(GcodeReceiver_Uart_Module[GCODERECEIVER_SELECTED_MODULE_IDX].Uart_Channel, CountOfDataBytes, COUNT_OF_DATA_BYTES_NUMBER);
	Uart_Recive(USART_2, CountOfDataBytes, COUNT_OF_DATA_BYTES_NUMBER);
	return _SUCCESS;
}
